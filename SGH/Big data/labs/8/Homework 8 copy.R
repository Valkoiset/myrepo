...
lift.chart(c("GLM.2","MixedWesbrook","RPART.3"), valid, "Y", 0.01, 
...
NNET.2 <- Nnet(WESBROOK ~ ., data=train, decay=0.10, 
               size=4)
...
lift.chart(c("GLM.2","RPART.3","NNET.2" ), train, "Y", 0.01, 
           "cumulative", "Zbiór estymacyjny")

# Validation subset

lift.chart(c("GLM.2","RPART.3","NNET.2"), valid, "Y", 0.01, 
           "cumulative", "Zbiór walidacyjny")

# Building new network with 2 neurons in hidden layer

NNET.3 <- Nnet(WESBROOK ~ ., data=train, decay=0.10, 
               size=2)

# Lift charts - estimation subset

lift.chart(c("GLM.2","RPART.3","NNET.2","NNET.3" ), train, "Y", 0.01, 
           "cumulative", "Zbiór estymacyjny")

# Lift charts - validation subset

lift.chart(c("GLM.2","RPART.3","NNET.2","NNET.3"), valid, "Y", 0.01, 
           "cumulative", "Zbiór walidacyjny")
...
# Lift charts - estimation subset
lift.chart(c("GLM.2","RPART.3","NNET.2","NNET.3","NNET.4" ), train, "Y", 0.01, 
           "cumulative", "Zbiór estymacyjny")
...
lift.chart(c("GLM.2","RPART.3","NNET.4"), train, "Y", 0.01, 
           "cumulative", "Zbiór walidacyjny")
