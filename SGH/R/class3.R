...
lines(ex, ey)
title("Probability density function of data in the data set")
...
# (1) margin of graph
par(mai = c(1, 1, 1, 1), omi = c(0, 0, 0, 0))
# (2) 3 sets with 3 different types of distribution of random numbers
set.seed(591)
xx1 <- rnorm(20, mean = 3, sd = 3.6)
xx2 <- rpois(40, lambda = 3.5)
xx3 <- rchisq(31, df = 5)
# (3)  
# 1) vector with a mean of 3 variables; 2) vector with
# standard deviation of 3 variables; 3) list of 3 vars
mean1 <- c(mean(xx1), mean(xx2), mean(xx3))
sd1 <- c(sd(xx1), sd(xx2), sd(xx3))
data1 <- list(xx1, xx2, xx3)
# (4) 1) minimum values of 3 vectors; 2) maximum values of vectors
xmin1 <- min(xx1, xx2, xx3) - 1
xmax1 <- max(xx1, xx2, xx3) + 1
# (5) plotting of datasets on stripchart
stripchart(data1, method = "jitter", jit = 0.3, vert = T, 
           pch = 1, cex = 0.4, ylim = c(xmin1, xmax1),  
           group.names = c("Group-1", "Group-2", "Group-3"))
title("Illustration of 3 types of distribution of random numbers")
# (6) 
# 1) mean +- sd; angle of arrow head = 45 degrees. length is the length
# of the arrow head
arrows(1:3, mean1 + sd1, 1:3, mean1 - sd1, angle = 45, 
       code = 3, length = 0.07)
# 2) mean +- 2*sd; angle of arrow head = 30 degrees
arrows(1:3, mean1 + 2 * sd1, 1:3, mean1 - 2 * sd1, 
       angle = 30, code = 3, length = 0.07)
# 3) mean - sd/10; angle = 90; arrow looks like a straight line
arrows(1:3, mean1 + 0.01 * sd1, 1:3, mean1 - 0.01 * sd1, 
       angle = 90, code = 3, length = 0.12)

# Exercise 4. The same as third implemented by box chart.
...
# (2)
set.seed(591)
# creating 3 variables with random numbers with 3 types of distribution
xx1 <- rnorm(20, mean = 3, sd = 3.6)
xx2 <- rpois(40, lambda = 3.5)
...
box1 <- boxplot(xx1, xx2, xx3, names = c("Group-1", "Group-2",
                                         "Group-3"), cex = 0.7)
title("Illustration of 3 types of distribution using box plot")
# (4)
print("box1$stats")
print(box1$stats)
...
title("strip chart")
legend(0.75, 2, c("y", "y_2"), pch = c(0, 16))
# (4)
par(new = T)
# (5)
...
# (1) set parameters of overall plot
par(mai = c(1, 1, 1, 1), omi = c(0, 0, 0, 0))
# (2) creating vector
yy <- c(50,30,40)
# (3) name every value in the vector
name1 <- c("data-a", "data-b", "data-c")
# (4) creating piechart from the vector
pie(yy, labels = name1, col = c("red","green","skyblue"))
# (5) adds new parameter to existing pie chart
par(new = T)
# (6) setting the parameters of new section
par(mai=c(2, 2, 2, 2))
# (7) adding new vector
yy2 <- c(50, 20, 10, 20, 20)
# (8) adding name to every value of vector
name2 <- c("data-a1", "data-b1", "data-b2", "data-c1", 
           "data-c2")
# (9) creating one more pie chart from yy2 vector
