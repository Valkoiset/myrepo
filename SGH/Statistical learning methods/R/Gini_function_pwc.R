...
#read data
dane <- read.csv("sample_data.csv")

#split dataset and create test, train, validation dataset

# model1
mod <- glm(DefFlag~ Age  + Monthly_Income + Monthly_Spendings +  Job_type, data = ..., family = binomial())
...
Prediction <- data.frame(Application_ID = predict$Application_ID, Score = prognFinal)
...
#save file
write.csv(final, "Output.csv", row.names = T)
