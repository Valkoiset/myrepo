...
/*deklaracja katalogu gdzie bêd¹ le¿a³y dane wyjœciowe*/
libname wyj 'g:\sas\project';
libname t (wyj);
%let kat_tree=%sysfunc(pathname(wyj));
%put &kat_tree;

/*deklaracja zmiennej celu*/
%let tar=default;
/*deklaracja zmiennej celu*/

libname dane 'g:\sas\proje';

/*deklaracja zbioru danych wejœciowych do binningu*/
%let em_data_variableset=dane.Merg3;
%let zb=dane.Merg3;
/*deklaracja zbioru danych wejœciowych do binningu*/

/*wybór zmiennych do binningu*/
proc sql;
	select upcase(name) into :zmienne_int_ord separated by ' '
...
/*min_il jako minimalna liczebnoœæ liœci */
/*%let min_il=;*/
/*min_il jako minimalna liczebnoœæ liœci */

/*ALBO minimalny udzia³ obserwacji w liœciu*/
%let min_percent=3;
data _null_;
...
/*maksymalna liczba kategorii -1*/
%let max_il_podz=2;
/*maksymalna liczba kategorii -1*/

/*kryterium albo entropia (h) albo gini (g);*/
%let crit=h;
/*kryterium albo entropia (h) albo gini (g);*/
...
%macro zrob_podz(zm);
	...
	proc means data=merg3 nway noprint;
		var &zm;
